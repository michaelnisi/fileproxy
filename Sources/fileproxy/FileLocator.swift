...
struct FileLocator: Codable {
  ...
  init?(identifier: String, url: RemoteURL) {
  ...
  static func targetDirectory(identifier: String) throws -> URL {
    ...
    guard
      let p = parent,
      let target = URL(string: identifier, relativeTo: p) else {
      throw FileProxyError.targetRequired
    }
  ...
  private func targetURL() throws -> URL {
    let target = try FileLocator.targetDirectory(identifier: identifier)
